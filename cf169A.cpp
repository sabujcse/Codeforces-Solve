...
int  main()
{
    long long  resturent, lunce_time;
    cn>>resturent>>lunce_time;
    long long max_result = -999999999;
    long long  fi, ti;
    bool flag = false;
    for( int i = 1; i<=resturent; i++){
        cn>>fi>>ti;
        if( ti<= lunce_time ){
            if(max_result<fi)
                max_result = fi;
            ...
            long long xx = fi -(ti-lunce_time);
            max_result = max(max_result,xx);
        }
    }
